...
class Encoder(nn.Module):
    ...
    def __init__(self, hparams, input_size=28 * 28, latent_dim=20):
        ...
        self.encoder = nn.Sequential(
            nn.Linear(self.input_size, self.hparams["nn_hidden_layer1"]),
            nn.ReLU(inplace=True),
            nn.Linear(self.hparams["nn_hidden_layer1"], self.hparams["nn_hidden_layer2"]),
#             nn.ReLU(inplace=True),
#             nn.Linear(self.hparams["nn_hidden_layer2"], self.hparams["nn_hidden_layer3"]),
         )
...
#                         nn.Linear(self.hparams["nn_hidden_layer3"], self.hparams["nn_hidden_layer2"]),
#                         nn.ReLU(inplace=True),
                        nn.Linear(self.hparams["nn_hidden_layer2"], self.hparams["nn_hidden_layer1"]),
                        nn.ReLU(inplace=True),
                        nn.Linear(self.hparams["nn_hidden_layer1"], self.output_size),
#                         nn.Sigmoid()
                    )
        ########################################################################
        #                           END OF YOUR CODE                           #
...
class Classifier(pl.LightningModule):
    ...
    def __init__(self, hparams, encoder, train_set=None, val_set=None, test_set=None):
        ...
        if "nn_hidden_layer3" in self.hparams:
            self.model = nn.Sequential(
                               nn.Linear(self.hparams["nn_hidden_layer3"], self.hparams["num_classes"]),
                               nn.ReLU(inplace=True),
                            )
        if "nn_hidden_layer2" in self.hparams:
            self.model = nn.Sequential(
                               nn.Linear(self.hparams["nn_hidden_layer2"], self.hparams["num_classes"]),
                               nn.ReLU(inplace=True),
                            )
